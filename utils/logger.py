...
class Logger:
    ...
    def __init__(self, logdir, rank, type='tensorboardX', debug=False, filename=None, summary=True, step=None):
        ...
        logging.basicConfig(filename=filename, filemode='a', level=logging.INFO, format=f'%(levelname)s:rank{rank}: %(message)s')
    ...
    def save_log(self, names, values):
        with open(f"{self.logdir}/results_{self.step}.txt", "w") as text_file:
            for name, value in zip(names, values):
                text_file.write(f"{name}:{value}\n")

    def save_array(self, fn, value):
        import numpy as np
        with open(f"{self.logdir}/{fn}_{self.step}.txt", 'wb') as f:
            np.save(f, value)

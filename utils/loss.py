...
class UnbiasedClassSimilarityWeightedKD(nn.Module):
    ...
    def forward(self, preds_S, preds_T, batch_prototypes, seg, mask, classes=None):
        ...
        batch_prototypes.detach()
        preds_T.detach()
        new_cl = preds_S.shape[1] - preds_T.shape[1]
        #preds_S = preds_S.narrow(1, 0, preds_T.shape[1])
        assert not torch.isnan(batch_prototypes).any(), "NaN in prototype"
        #assert preds_S[0].shape == preds_T[0].shape, 'the output dim of teacher and student differ'
        ...
        seg = seg.view(B*H*W)[mask] # T
        T = seg.size(0)
        #preds_S = preds_S/self.T
        #preds_T = preds_T/self.T


        new_bkg_idx = torch.tensor([0] + [x for x in range(preds_T.shape[-1], preds_S.shape[-1])]).to(
            preds_S.device
        )

        den = torch.logsumexp(preds_S, dim=-1)  # B, H, W
        outputs_no_bgk = preds_S[:, 1:-new_cl] - den.unsqueeze(dim=1)  # B, OLD_CL, H, W
        outputs_bkg = torch.logsumexp(
            torch.index_select(preds_S, index=new_bkg_idx, dim=-1), dim=-1
        ) - den  # B, H, W

        labels = torch.softmax(preds_T, dim=-1)  # B, BKG + OLD_CL, H, W
        ...
        loss = self.T**2*(labels[:, 0] * outputs_bkg +
                (labels[:, 1:] * outputs_no_bgk).sum(dim=-1)) / preds_T.shape[-1]
...
class SoftContrastiveLoss(nn.Module):
    ...
    def forward(self, labels, features, mask=None, prototypes=None):
                ...
                if self.lfc_orth_maxonly:
                    ...
                    loss_separationclustering = torch.sum(vectorial_products) / \
                                                (vectorial_products.shape[0]*vectorial_products.shape[1] -
                                                    cl_present.shape[0])
        loss_features_clustering *= self.lfc

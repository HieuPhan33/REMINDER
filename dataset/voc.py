...
class VOCSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        name = os.path.splitext(os.path.split(self.images[index][0])[-1])[0]
...
class VOCSegmentationIncremental(data.Dataset):
    ...
    def save_pred(self, inp, sv_path, name, image=None):
        import os
        save_img = Image.fromarray(inp)
        if image is not None:
            image = Image.fromarray(image)
            save_img = Image.blend(image.convert('RGBA'), save_img.convert('RGBA'), alpha=0.75)

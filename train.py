...
class Trainer:
    def __init__(self, model, model_old, device, opts, trainer_state=None, classes=None, step=0, prototypes=None):
        ...
        if self.bce:
            ...
            ) # For a background pixel, a new model can either predict as background or old classes
        ...
        if opts.unkd:
            ...
            # For new class, the new model can either predict it as background or a new class, when compared with background
            # class prediction by the old model.
    ...
    def validate(self, loader, metrics, ret_samples_ids=None, logger=None, end_task=False):
        ...
        with torch.no_grad():
            ...
            torch.distributed.reduce(reg_loss, dst=0)
            #torch.distributed.reduce(self.current_prototypes, dst=0)
...
def features_distillation(
    ...
    for i, (a, b) in enumerate(zip(list_attentions_a, list_attentions_b)):
        ...
        if not torch.isfinite(layer_loss).all(): print(a, b)

...
def run_step(opts, world_size, rank, device):
    ...
    if rank == 0 and opts.sample_num > 0:
        ...
        #sample_ids = ['2008_003195','2008_003307','2008_003353','2008_003568','2008_004644','2008_004691','2008_005710','2008_006633','2008_008709','2009_001488']
    else:
        sample_ids = None
    ...
    if TRAIN:
        ...
        for cur_epoch in range(current_epoch, opts.epochs):
            ...
            if (cur_epoch + 1) % opts.val_interval == 0:
                ...
                for k, (img, target, lbl, name) in enumerate(ret_samples):
    ...
    if rank == 0 and TRAIN:  # save best model at the last iteration
        ...
        # if opts.step > 0:
        #     # Write prototypes of prevous steps
    ...
    if True: #TRAIN:
        ...
        del checkpoint
        classes = tasks.get_per_task_classes(opts.dataset, opts.task, opts.step)
        trainer = Trainer(model, None, device=device, opts=opts, step=opts.step, classes=classes)
    ...
    if opts.rebal_kd > 0:
        ...
        if opts.step > 0:
            ...
            if opts.dataset == 'cityscapesDomain':

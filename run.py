...
def run_step(opts, world_size, rank, device):
    ...
    if rank == 0:
        logger = Logger(
            logdir_full, rank=rank, debug=opts.debug, summary=opts.visualize, step=opts.step, filename=f"{logdir_full}/log.txt"
        )
    else:
        logger = Logger(logdir_full, rank=rank, debug=opts.debug, summary=False, filename=f"{logdir_full}/log.txt")
    ...
    opts.checkpoint = f'{opts.checkpoint}/{opts.dataset}'
    ...
    if opts.step > 0:
        ...
        if os.path.exists(f"{opts.checkpoint}/{task_name}_{opts.name}_prototypes_{opts.step-1}.pth"):
            ...
            print(f"Prototype files not found in {opts.checkpoint}/{task_name}_{opts.name}_prototypes_{opts.step-1}.pth")
    ...
    if TRAIN:
        ...
        for cur_epoch in range(current_epoch, opts.epochs):
            # =====  Train  =====
            model.train()
            ...
            if (cur_epoch + 1) % opts.val_interval == 0:
                ...
                print(f"Epoch {cur_epoch}: Val_MeanIoU {val_score['Mean IoU']}")
    ...
    if rank == 0 and TRAIN:  # save best model at the last iteration
        ...
        save_ckpt(
            ...
            scheduler, opts.epochs, best_score
        )
        logger.info("[!] Checkpoint saved.")
    ...
    sv_dir = f'results/{opts.date}_{opts.dataset}_{opts.task}_{opts.name}'
    ...
    for k, (img, target, lbl) in enumerate(ret_samples):
        ...
        train_dst.save_pred(target, sv_path=sv_dir, name=f'Sample_{k}_step{opts.step}', image=img)
        train_dst.save_pred(lbl, sv_path=sv_dir, name=f'Sample_{k}_step{opts.step}', image=img)
    ...
    logger.save_array(fn="CM", value=val_score['CM'])
